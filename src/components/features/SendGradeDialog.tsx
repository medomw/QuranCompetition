...
const SendGradeDialog = ({ app, open, onClose, getLevelName }: SendGradeDialogProps) => {
  ...
  const drawCard = useCallback(() => {
    ...
    const drawCornerOrn = (x: number, y: number, flip: boolean) => {
      ...
      ctx.fillStyle = g; ctx.font = '52px serif';
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('❧', 30, 30); ctx.restore();
    };
    drawCornerOrn(20, 20, false); drawCornerOrn(W - 20, 20, true);
    drawCornerOrn(20, H - 20, true); drawCornerOrn(W - 20, H - 20, false);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      ctx.beginPath(); ctx.moveTo(cx, ey); ctx.lineTo(cx + Math.cos(a) * 57, ey + Math.sin(a) * 57); ctx.stroke();
      ctx.restore();
    }
    ...
    const nameLabel = gender === 'male' ? 'المتسابق' : 'المتسابقة';
    ...
    ctx.fillText(app.full_name, W / 2, 438); ctx.restore();
    ...
    if (score && hasRank) {
      drawPill(`الدرجة: ${score} / 100`, W/2 - 200, 320, '#FFD700');
      drawPill(`المركز: ${rank}`, W/2 + 200, 320, '#C9A84C');
    } else if (score) {
      drawPill(`الدرجة: ${score} / 100`, W/2, 360, '#FFD700');
    } else if (hasRank) {
      drawPill(`المركز: ${rank}`, W/2, 360, '#C9A84C');
    }
  ...
  useEffect(() => {
    if (open) {
      setScore(''); setRank('none'); setGender('male');
      setImageReady(false); setCapturedFormImage(null);
    }
  }, [open, app]);

  const handleCameraCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) { setCapturedFormImage(file); toast.success('تم التقاط صورة الاستمارة بنجاح'); }
    e.target.value = '';
  };

  const getCardBlob = (): Promise<Blob | null> =>
    new Promise((resolve) => { const c = canvasRef.current; if (!c) { resolve(null); return; } c.toBlob(resolve, 'image/png'); });
  ...
  const buildMessage = (imageUrl?: string, formUrl?: string) => {
    ...
    if (imageUrl) msg += `\n\n🖼️ بيان الدرجات (اضغط لمشاهدته وحفظه):\n${dl(imageUrl, 'بيان_الدرجات.png')}`;
    if (formUrl)  msg += `\n\n📄 صورة الاستمارة (اضغط لمشاهدتها وحفظها):\n${dl(formUrl, 'استمارة_المسابقة.png')}`;
    return msg;
  };

  const handleSend = async () => {
    if (!app?.whatsapp) { toast.error('لا يوجد رقم واتساب لهذا المتسابق'); return; }
    if (!imageReady) return;
    setSending(true); toast.info('جاري رفع الصور...');

    const cardBlob = await getCardBlob();
    let cardUrl: string | null = null;
    if (cardBlob) { cardUrl = await uploadToStorage(cardBlob, `${app.id}_card.png`); if (!cardUrl) toast.error('تعذّر رفع بيان الدرجات'); }

    let formUrl: string | null = null;
    if (capturedFormImage) {
      const fp = `forms/${Date.now()}_${app.id}_form.png`;
      const { error: fe } = await supabase.storage.from('results').upload(fp, capturedFormImage, { contentType: capturedFormImage.type, upsert: true });
      if (!fe) { const { data } = supabase.storage.from('results').getPublicUrl(fp); formUrl = data.publicUrl; }
      ...
      <DialogContent className="max-w-2xl w-full max-h-[95vh] overflow-y-auto" dir="rtl">
        <DialogHeader>
          <DialogTitle className="text-emerald-800 text-xl flex items-center gap-2">
            ...
            إرسال بيان الدرجات — {app.full_name}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-5 py-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-1">
              ...
              <Input type="number" min="0" max="100" placeholder="مثال: 85"
            ...
            <div className="space-y-1">
              ...
              <div className="flex gap-2 h-11">
                <button type="button" onClick={() => setGender('male')}
                  className={`flex-1 rounded-lg border-2 text-sm font-semibold transition-all ${gender === 'male' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-700 border-slate-300 hover:border-emerald-400'}`}>
                  ذكر
                </button>
                <button type="button" onClick={() => setGender('female')}
                  className={`flex-1 rounded-lg border-2 text-sm font-semibold transition-all ${gender === 'female' ? 'bg-pink-500 text-white border-pink-500' : 'bg-white text-slate-700 border-slate-300 hover:border-pink-400'}`}>
                  أنثى
                </button>
              </div>
            </div>
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 text-sm text-emerald-800 space-y-1">
            ...
            <p>١. يتم رفع الصور إلى الخادم وتحويلها إلى روابط</p>
            <p>٢. تُضاف الروابط في رسالة واتساب — المستقبل يضغط الرابط لمشاهدة البيان وحفظه</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Button onClick={handleSend} disabled={!app.whatsapp || !imageReady || sending}
